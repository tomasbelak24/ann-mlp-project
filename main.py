...
hyperparams_part2 = {
    'lr_schedule': [None, 'linear', 'exponential'],
...
def perform_grid_search(hyperparams, fixed_params=None):
    ...
    with open("all_model_results.csv", "a", newline="") as f:
        ...
        for i_model, values in enumerate(product(*hyperparams.values()), start=1):
            ...
            use_early_stopping = hp.get('patience') is not None
            ...
            train_CEs, train_REs, val_CEs, duration = model.train(
                ...
                early_stopping=use_early_stopping, patience=patience, delta=hp.get('delta', 0), lr_schedule=lr_schedule,
                live_plot=False
            )
...
train_CEs, train_REs, _, duration = model.train(inputs, labels, None, None, alpha=best_params_part2['alpha'], eps=best_params_part2['eps'], early_stopping=False, patience=None, live_plot=False)
